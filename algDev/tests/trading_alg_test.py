...
def run_test_one():
    ...
    ta = TradingAlgorithm(tickers, features, type = 'svm', data_lookback_period = 15, label_lower_threshold = -0.05, label_upper_threshold = 0.005, label_period = 10, data_splits = [0.8, 0.2], cnn_split=0, model_params ={'gamma':10,"C":100}, verbose=True)
...
def build_confusion_matrix():
    ...
    tickers = ['AAPL']
    features = ['macd_9_18','closes','prings','kst','kstTrix', 'rsi']
    ta = TradingAlgorithm(tickers, features, type = 'svm', data_lookback_period = 15, label_lower_threshold = -0.15, label_upper_threshold = 0.015, label_period = 10, data_splits = [0.8, 0.2], cnn_split=0, model_params ={'gamma':10,"C":100}, verbose=True)
    print(ta.models[0].accuracy)
